...
開始時に`Teams通知: 有効 / 無効`を選択する。未指定は`無効`とし、今回の実行だけに適用する。この補助Workflow単体では通知せず、親の計画または実装Workflowの完了時だけ通知する。
...
このWorkflowを単独で実行し、`Teams通知: 有効`を選択した場合は、Learning Quality Gateを`/artifact-quality-gate.md`で`PASS`と判定し、Skill `loop-verifier`が`APPROVE`になった後だけ`/teams-completion-notification.md`を`workflow`モードで実行する。親Workflow内の補助実行では親の完了通知へ委ねる。
