...
開始時に`Teams通知: 有効 / 無効`を選択する。未指定は`無効`とし、今回の実行だけに適用する。`有効`の場合でも最終品質ゲートと独立検証がPASSした時だけ通知する。
...
Skill `loop-budget`で予算と停止条件を定義する。実行前に環境、コンテキスト、ツール、観測可能性、制約、評価器が揃っているか確認する。
...
完了には品質ゲートPASSとSkill `loop-verifier`の`APPROVE`を必要とする。

`Teams通知: 有効`の場合、完了条件を満たした後に`/teams-completion-notification.md`を`workflow`モードで実行する。停止、予算超過、REJECT、ESCALATE_HUMANでは通知しない。
