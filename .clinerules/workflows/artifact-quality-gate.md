...
- HTML: `/html-artifact-checker`
- ソースコード: `/review-loop` と `/loop-verifier`
- 不明な根拠: `/unknown-list-extractor`
- 実装修正: ユーザーが修正を依頼している場合だけ `/implementation-loop`

その他の形式は `/artifact-quality-gate` のチェックリストに従い、利用可能なparser、schema、lint、build、test、DB、ブラウザ検証を実行する。
...
PASS後に `/memory-bank-updater` を実行する。再利用可能な訂正、失敗パターン、設計判断、性能測定、検証済みの改善がある場合は `/continuous-project-learning.md` へ進み、その後必要に応じて `/template-commit-workflow` または `/git-commit-workflow` へ進む。
