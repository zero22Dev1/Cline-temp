...
開始時に`Teams通知: 有効 / 無効`を選択する。未指定は`無効`とし、今回の実行だけに適用する。このWorkflow単体では通知せず、親の計画または実装Workflowが完了ゲートを通過した時だけ通知する。
...
セッションを越えて必要な情報だけをSkill `memory-bank-updater`へ渡す。仕様は`docs/specs/`、判断はADR、不明点は`docs/ai/unknowns/`、学習候補は`/continuous-project-learning.md`へ分離する。

## Step 7: Final Context Audit

完了前に最新依頼、Git差分、完了条件、検証結果、未解決事項を照合する。成果物の品質は`/artifact-quality-gate.md`とSkill `loop-verifier`で判定し、Context Checkpointの自己申告だけで完了にしない。

このWorkflowを単独で実行し、`Teams通知: 有効`を選択した場合は、Final Context AuditのQuality Gateが`PASS`、Skill `loop-verifier`が`APPROVE`になった後だけ`/teams-completion-notification.md`を`workflow`モードで実行する。親Workflow内の補助実行では親の完了通知へ委ね、途中通知しない。
