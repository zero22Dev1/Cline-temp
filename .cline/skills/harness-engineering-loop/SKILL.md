...
`/loop-budget`で最大イテレーション、時間、ツール、コスト、停止条件を決める。既定の自動修正上限は3回とし、長時間ループは明示的な予算を使用する。
...
`/artifact-quality-gate.md`と`/loop-verifier`を使用する。生成担当の説明を根拠にせず、要件、成果物、テスト結果、既存仕様を比較する。
