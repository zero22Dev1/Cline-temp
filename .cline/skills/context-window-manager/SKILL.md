...
不明点が完了条件や挙動を変える場合は、読み込みを増やす前に`/adaptive-deep-planning.md`またはSkill `grill-with-docs`を使う。
...
長期的に必要な内容だけをSkill `memory-bank-updater`へ渡す。一時的なCheckpointをmemory-bankへ大量保存しない。
