...
1. `/grill-with-docs` で目的、利用者、入出力、正常系、異常系、業務ルール、非機能要件、対象外、完了条件を整理する。
2. `/unknown-list-extractor` で事実、仮定、Unknown、Blocking Questionを分離する。
...
2. `/legacy-source-spec-writer` を必要に応じて使い、関連コードと現行挙動を調査する。
3. 呼び出し元・先、DB、API、画面、認証、ログ、テスト、運用への影響を `High / Medium / Low` で整理する。
4. `/unknown-list-extractor` で未確認事項を分類する。
...
成果物を生成または実装した後は `/artifact-quality-gate` を実行する。
